...
│        removal-only: min(total, money), never rejected       │       │                                │
...
│   ├── hooks/                    # Derived-state helpers for the HUD/stats panel
│   │   ├── laborStatus.ts        # Employment/unemployment HUD text from labor-market scalars
│   │   ├── budgetStatus.ts       # Net-flow/funded-%/warning HUD text from budget scalars
│   │   ├── sampleStats.ts        # Stats-panel time-series sampling reducer
│   │   └── useStatsHistory.ts    # React hook wrapping sampleStats for the stats panel
...
│   │   ├── serviceFunding.ts     # Pure service-funding-ratio helpers (fundingPerMille)
...
5. On a month-boundary day (`day % DAYS_PER_MONTH === 0`), settles a month at the pre-growth population: tax income (`TAX_PER_POP` per resident) minus structure and road upkeep (per-`StructureType` `*_UPKEEP` constants via `structureUpkeep`, plus `ROAD_UPKEEP` per road tile), through `earn`/`trySpend` — the treasury floors at 0 and unpayable upkeep is forgone, not carried as debt. The settlement also records how much of that upkeep it actually paid, as a per-mille ratio `r = due > 0 ? paid/due : 1` (`paid` is the clamped charge; one uniform ratio covers structures AND roads, no priority order between them) — persisted as `sf` in the save envelope, full (`1000`) on New City. Because this settlement precedes the growth pass below, a tick that is both a month boundary and a growth tick is already gated by that same month's payment.
6. On growth ticks (`tickCount % ZONE_GROWTH_INTERVAL === 0`), first runs an **abandonment sweep** over every building (added service-v8 / save v18 — `Building.abandoned: boolean`): if land value at the anchor no longer supports the building's level, it flips `abandoned = true`; if a previously-abandoned building's anchor land value now supports its level again, it flips back to `false`. The value it reads is the anchor's **uncongested** land value (`LandValueMap.getUncongestedValue`), not the congested `getValue` every growth gate below reads, and the verdict reads neither congestion nor the service-funding ratio; why is documented once at the sweep in `World.tick`. Either flip freezes the building for the rest of this tick — recorded in a `frozenThisTick` set, not the live `abandoned` field, so a building that just re-occupied is still skipped by growth/merge below. That freeze skip sits after `age += 1`, so a derelict with road and power keeps aging and does not re-earn its cooldown from zero on recovery. Abandoned buildings are excluded from population, jobs, demand, and the labor market; the sweep runs before demand is (re)computed so this tick's growth pass already sees demand net of the buildings that just went derelict. The render layer shows a derelict tint with dark windows.
   ...
   - No building yet on this tile → create a **level-1** building, gated on demand above `GROWTH_DEMAND_THRESHOLD` for that zone type, power at the seed tile, a frontage road (`pickSeedFrontage` scans distances 1..4 cardinally, tie-breaking S > E > W > N, so a seed may sit up to four cells behind the road), and a valid lot (`greedyDepthLot`) — NOT gated on upkeep funding: spawn keeps widening the tax base even while the treasury is short
   - Building already exists → gated first on `hasFrontageRoadAccess` + power (a building that loses either stops aging entirely; aging itself is likewise not gated on upkeep funding), then, additionally requiring the latest settlement to have paid upkeep in full (`upkeepFunded`, step 5 above — blocked while any upkeep went unpaid), one of three mutually exclusive branches, checked in this order:
     ...
     - **structure is capped and at `ZONE_MAX_LEVEL`** → density bump, gated on demand ≥ `DENSITY_DEMAND_BAR[type]` (the per-type bar, see "Gate summary" below), its own cooldown, `density < maxDensityForLot(lot, frontage)` (a 1-wide lot stops at tier 1; only an assembled ≥2-wide lot, reachable by merging, reaches tier 2 — no SIMULATION path can ever exceed the cap: spawns start at density 0, this gate blocks every increase, and a merge output's cap is never below its inputs' (pinned in `mergePolicy.test.ts`). A save carrying an over-cap density from before this cap existed is rejected at load — both the envelope version (`WORLD_SAVE_VERSION` 18 -> 19 bump) and, independently, `mapSerialization.ts`'s per-building density-vs-lot-width check. The dev-only `devApi` seeding path can still construct an over-cap building directly in memory, same as it can construct any other otherwise-unreachable state), water, the same four service coverages, AND an explicit anchor land-value gate at `LEVEL_THRESHOLDS[ZONE_MAX_LEVEL]` read from the CONGESTED field, exactly like the two rungs above it. That gate does real work now that the sweep reads the uncongested value: a level-5 building whose anchor is suppressed purely by traffic survives the sweep and reaches this branch below the threshold, and this is what holds it. Density is a growth rung, so congestion freezes it the same way it freezes level-up and structure-grow — that reason alone justifies the gate. A second, weaker one: a bump raises the building's worker or job capacity, which CAN deepen the congestion that suppressed the anchor, though only if the labor matcher actually fills that added capacity — added workers and added job slots alike load no roads until they are matched (`laborMarket.ts`)

     Below `ZONE_MAX_LEVEL` this ordering is bit-identical to before (structure-grow already preceded level-up there). The one behavioural change is at max level: a building whose structure can still extend now does that instead of densifying. The gain is keyed on lot WIDTH along the frontage axis, not merge generation count: `structureDepthCap` is `max(MIN_STRUCTURE_DEPTH_CAP, lot width)` = `max(2, width)`, so a 2-wide lot's cap is still `max(2, 2) = 2` (nothing to spend), while every lot 3 wide and up carries a cap above the floor. In practice reaching width 3 still takes a second merge, because `greedyDepthLot` walks only the depth axis and so every unmerged lot is exactly 1 wide along the frontage: a 1+1 merge only reaches width 2 (no headroom yet), and width 3 needs a further 2+1 merge on top of that. Whether the extra headroom is actually spendable still depends on the lot's own depth being greater than that raised cap — a lot no more than 2 cells deep is already structure-capped at 2 regardless of width and gains no room from a width-only merge, as measured on the previous cycle's playtest: its only 4-wide lot was 2 cells deep and never extended in 7797 ticks.

   Separately, after the per-tile loop, a width-axis **merge** pass considers every pair of adjacent, same-type, same-frontage buildings — the whole pass is skipped while the latest settlement left any upkeep unpaid (`upkeepFunded`, step 5 above), since a merge is densification too (only an assembled lot reaches the top density tier). `canMerge` treats merging as the LAST redevelopment rung, not a response to a demand spike: a pair merges only once both parcels are BUILT OUT — both at `ZONE_MAX_LEVEL` (5), both at the density tier their own lot width allows (`maxDensityForLot`), and both with structures that can no longer extend (`canExtendStructure` false on each side) — with demand merely positive (`> GROWTH_DEMAND_THRESHOLD`, not a density-tier spike), since a parcel that has exhausted every cheaper way to grow has nothing left to answer demand with. `canMerge` also keeps its existing geometry checks (equal lot depth, frontage-edge alignment, width-axis adjacency, 4×4 lot cap); the shape gate itself now requires only equal **structureRect DEPTH** (`structureDepth` in `zoneGrowth.ts`: `sr.h` for N/S frontage, `sr.w` for W/E) — the WIDTH axis is free. Under the unchanged `mergedW <= 4 && mergedH <= 4` lot cap the admitted width pairs are 1+1, 1+2, 2+2, and 1+3. Because both sides are built out, `level` is automatically equal — nothing in the sim carries a level past `ZONE_MAX_LEVEL` (5) — which also closes an NW-anchor defect, since the merged building's anchor is always one of the two originals', whose level the abandonment sweep already verified this same tick against the uncongested land value. `density` is no longer automatically equal now that width is free — a mixed-width pair (1+2, 1+3) can arrive at different tiers on each side, and `mergedBuildingShape` takes the max — so capacity conservation now holds as an equality only when the two sides share a tier (1+1, 2+2); a mixed-tier pair instead revalues the narrower side at the wider lot's tier, so `buildingCapacity(merged) >= buildingCapacity(a) + buildingCapacity(b)`, an intended gain from land assembly, not a defect (see the built-out, equal-depth, and density-cap gate comments in `canMerge`, `game/core/mergePolicy.ts`, for the full derivation, including which mixed-width pairs the depth gate actually admits in practice — pinned as a documented gap in `mergePolicy.test.ts`). A first-generation (1+1, 2-wide) merge still unlocks density tier 2 via `maxDensityForLot`, which its 1-wide inputs could never reach alone — level-5 ceiling 200 vs. 140 for the unmerged pair (≈ +43%); reaching structure-depth headroom instead takes a second-generation merge (see the structure-grow discussion above) and a lot deep enough to spend it.

   The growth pass reads `landValue` as a frozen snapshot taken at step 3. Power gates initial zone spawn (checked on the seed tile, since no footprint exists yet) *and* every existing-building mutation — an unpowered building does not even age; water gates the mutations themselves at the footprint (binary): structure growth, level-up, density, and merge. Level-up, structure-grow, and the density bump additionally require police, fire, hospital, AND school coverage at the building anchor tile (graded fields gate at the anchor, not the footprint) — those same four coverages also raise land value via the combined service term.
   ...
   **Dual role of services:** the four coverage services hard-gate level-up, structure growth, AND the density bump at the building anchor in `World` (all four must cover the anchor), AND contribute to land value via the combined service term. A coverage-map or traffic change marks land value dirty (dirty cascade).
   ...
   Gate summary: upkeep funded (last settlement paid in full) → level-up, structure-grow, density bump, merge; not spawn, not the sweep. power → initial spawn (seed tile) + all existing-building aging/growth/merge; water → structure growth, level-up, density, and merge at the footprint (binary); four coverage services → level-up, structure-grow, AND density bump at anchor (AND-gate); land value (road + diversity + service + park − congestion) → level-up/structure-grow AND the density bump at anchor via `LEVEL_THRESHOLDS`, all three against the same frozen congested snapshot (the density bump gates explicitly at `LEVEL_THRESHOLDS[ZONE_MAX_LEVEL]`; the abandonment sweep is the one reader that instead takes the uncongested value). Density is further gated by demand ≥ `DENSITY_DEMAND_BAR[type]` — the base ratio normalized against each type's labor-derived demand range (R 0.375, C/I 0.1875, derived from `COMMERCIAL_JOB_SHARE` in `Demand.ts`), commercial's retail arm intentionally sharing the C bar, the C/I gate opening just above a 12.5% unemployment rate — its own cooldown, water, and the lot-width tier cap (`maxDensityForLot`); merge by the built-out condition (both sides at `ZONE_MAX_LEVEL`, at their own lot-width density cap, and unable to extend further), demand merely positive (`> GROWTH_DEMAND_THRESHOLD`), cooldown, water, and the equal-`structureRect`-DEPTH shape gate — capacity never decreases across a merge, and gains exactly when the two sides' density tiers differ.
...
- **Labor market** (`game/core/laborMarket.ts` + `LaborMarketMap.ts`): every non-abandoned residential building supplies `buildingCapacity(b)` workers and every non-abandoned commercial/industrial building supplies `buildingCapacity(b)` job capacity.
  - *Shared unit.* `buildingCapacity` (`game/core/buildingCapacity.ts`: `structureRect.w * structureRect.h * level * DENSITY_CAPACITY_UNITS[density]`) is the one shared function `World.getPopulation`, `World.recomputeHappiness`, this labor market, `Demand.recompute`'s retail axis, and the jobs data-view overlay all sum — so simulated commuters, job slots, city population, and the HUD/overlay figures are always denominated in the same unit and can never drift apart — otherwise the sim would route a fraction of the residents it claims to have and every volume-calibrated knob downstream would be measured against the wrong magnitude.
  - *Road access.* Only buildings with road access become graph nodes. A road-less residential building's workers go straight to `unemployed`; a road-less C/I building's capacity still counts toward `jobsCapacity` but is never inserted into `capByNode`, so it can never be filled. Zero-worker (level-0) origins are skipped so they don't inflate `reachableUnfilledJobs`.
  - *Matching.* Matching is greedy nearest-with-overflow — a forward BFS from each origin's access node ranks reachable job nodes by road-hop distance, filling the nearest node with remaining capacity first and spilling over to the next when it fills. Capacity is global and consumed in deterministic origin order (ascending access-node index, then building id).
  - *Output.* Produces `CommuteFlow[]` (origin node → destination node → worker count) plus `employed`/`unemployed`/`jobsCapacity`/`jobsFilled`/`reachableUnfilledJobs` scalars.
  - *Demand severity.* `Demand.recompute` (`game/core/Demand.ts`) is now a stateless function of that same snapshot, not an additive nudge onto separate structural terms: it derives a single aggregate workplace severity from the shortfall between reachable vacancies and `unemployed` against the whole labor market — deadbanded below `DEADBAND_RATE`, saturating at `SATURATION_RATE`, and floored at `MIN_MARKET` (`20 * POPULATION_PER_TILE_LEVEL`) so one *modal* density-0 building-level of imbalance (`structureRect` area 2) still reads as partial severity — then splits that one severity `COMMERCIAL_JOB_SHARE` / `1 - COMMERCIAL_JOB_SHARE` between commercial and industrial so neither double-counts the same shortage.
  - *Per-zone drivers.* Commercial additionally answers an internal-market retail axis (its capacity share against `COMMERCIAL_CAPACITY_SHARE`, damped by labor availability), and residential is the larger of its own mirrored severity (a vacancy surplus rather than a shortage) or `MIGRATION_PRESSURE` — a separately named external growth driver damped to zero as unemployment approaches `MIGRATION_UNEMPLOYMENT_CUTOFF`, not a baseline folded into the formula — except when the labor market holds neither workers nor job capacity (`marketEmpty`), when residential instead reads the flat `BOOTSTRAP_RESIDENTIAL_DEMAND`. Commercial and industrial carry the same kind of external driver: a `workplaceFloor` of `WORKPLACE_PRESSURE`, scaled by the same staffing damper (`1 - resSeverity`) that gates the retail axis, combined by `max` outside the severity on both bars, and forced to exactly 0 whenever the labor market holds no workforce at all (nothing left to staff a new workplace).
  - *Resting point.* A fully employed, balanced city therefore reads R 0.10 / C 0.10 / I 0.10 — commercial and industrial no longer rest at zero there, each bar now resting on its own external driver once the labor-derived severities settle at zero, and each damping to exactly 0.00 only past its *own* cutoff: at net labor balance, R reaches 0.00 once unemployment hits `MIGRATION_UNEMPLOYMENT_CUTOFF` (away from balance a vacancy surplus still drives R through `resSeverity`, so past-cutoff R is not always zero), while C and I reach 0.00 once the reachable-vacancy surplus hits `SATURATION_RATE` (an empty workforce zeroes the floor and therefore industrial, but commercial can still read its retail axis there — the zero-workforce hamlet reads C ≈ 0.19). Without an external driver on all three, the all-zero state would be absorbing (an empty zone tile supplies no workers, jobs, or levels to escape it); with them, `MIGRATION_PRESSURE` and `WORKPLACE_PRESSURE` together keep all three spawn/level-up gates open at `GROWTH_DEMAND_THRESHOLD` while every labor-derived severity sits at zero.
  - *Pacing.* Demand only states a direction, never the brake — land value, power, water, the four service coverages, and the age cooldown do the actual pacing — and this stateless formula has no lagged census, no integrator/anti-windup valve, and no per-tile stochastic evaluation, so a large newly-zoned block can spawn in a single growth pass and overshoot before the next tick's labor recompute reflects it.
- **Traffic assignment** (`game/core/trafficAssignment.ts` + `TrafficMap.ts`): consumes the labor market's `CommuteFlow[]` and loads each flow's worker count onto every road tile along the *exact* shortest road path from its origin to its exact destination. Flows are grouped by destination and a single reverse BFS is run per distinct destination (not one shared multi-source BFS) — an overflow flow may have been matched to a farther job than the nearest one, so routing must stay per-destination-exact. Load is normalized against `TRAFFIC_CAPACITY` (`100 * POPULATION_PER_TILE_LEVEL`, still 500 — chained to the same per-structure-tile-per-level commuter unit `buildingCapacity` multiplies, not re-derived from `POPULATION_PER_LEVEL` in parallel, so modelling labor participation later cannot silently decalibrate it) into a `0..255` congestion value per road tile. It is calibrated from the un-normalized trip loads measured in two play-verified cities — a single-corridor city, whose mid-corridor tiles carry the whole employed population and so saturate, versus an ordinary lattice with jobs among the homes, whose busiest street carries only the handful of short commutes that overlap on it. Capacity sits in the gap: the corridor city reads near-saturated — un-clamped at the sizes measured, and clamping only once its employed population passes the capacity — and takes roughly one `LEVEL_THRESHOLDS` band of land-value penalty, enough to FREEZE the level-up, structure-grow, and density rungs for a marginal building along that corridor (the intended bite), while an ordinary layout is merely nudged and stays below every growth-gate margin measured there. Neither reading can abandon anything: congestion is absent from the value the abandonment sweep reads. The measured loads, the rejected alternatives, and the numbers live in the `TRAFFIC_CAPACITY` JSDoc, which is the single source for them. `TrafficMap.getCongestionIndex()` reduces the whole grid to a single congestion-intensity-weighted mean in `[0, 1]` (`Σc_i² / (255 · Σc_i)`, each tile's own congestion doubling as its weight) for the happiness KPI below.
- **Feedback into land value and happiness**: `LandValueMap.recompute` takes the `TrafficMap` as a fourth argument and subtracts a proximity-weighted congestion penalty (see the land-value formula above); `World`'s happiness formula subtracts `HAPPINESS_W_TRAFFIC * getCongestionIndex()` (0.15) from the three positive terms (land value × 0.5, employment × 0.3, and `HAPPINESS_W_BUDGET` × `budgetHealthScore` at 0.2 — see `World.getHappiness()`); `budgetHealthScore(money, monthlyIncome, monthlyUpkeep)` blends `BUDGET_W_STOCK` × treasury/`STARTING_FUNDS` with `BUDGET_W_FLOW` × a projected monthly net-flow score (1 at any surplus, 0 at a `BUDGET_DEFICIT_SPAN` deficit), both weights 0.5, derived each recompute from the same `monthlyTaxIncome`/`monthlyUpkeep` helpers the settlement uses (step 5 above) and not persisted; the employment term reads only the unemployment rate, clamped against `MIGRATION_UNEMPLOYMENT_CUTOFF`, so a job surplus (unfilled vacancies) reads full marks and only a worker surplus (unemployment) drags it down. The dirty-cascade edge is `markTrafficDirty()` → `dirtyLandValueAndHappiness()`, and `markLaborDirty()` routes through `markTrafficDirty()` so that edge fires from one place — giving the full chain `markLaborDirty()` → `markTrafficDirty()` → `dirtyLandValueAndHappiness()`, alongside the parallel `markDemandDirty()`. All cascades fire at **mark time**, never at recompute time, so a drain-on-read call between ticks (e.g. the data-view overlay's `getTrafficMap()`) never silently clears a pending land-value/happiness refresh. Each tick, traffic resolves (dirty-or-cadence, force-refreshing labor inside `recomputeTraffic`) before land value, and both are then frozen read-only snapshots for the rest of the tick (see the tick pipeline above) — so the full loop (traffic → land value → growth → buildings → labor → traffic) runs ACROSS ticks, never within one: growth-pass building changes only set dirty flags at the end of a tick, resolved at the start of the next (one-tick lag). What congestion can do inside that loop is bounded to growth: it gates level-up, structure-grow, and the density bump, and it feeds the happiness KPI and the data-view overlay — but it can never abandon a building, because the abandonment sweep reads the uncongested land value (step 6 above; the argument lives at the sweep in `World.tick`). Congestion therefore FREEZES growth where it bites rather than condemning; the growth-side loop is unchanged, and only its verdict-side edge — dereliction deleting the commuters whose congestion condemned the building — is cut. Level 1 remains a floor on abandonment in its own right (`maxSupportedLevel` floors at 1, so a level-1 building never abandons at any land value).
...
Invocation rule: `new World(W, H)` and `World.reset({ regenerate: true })` invoke the generator. Save-hydration callsites (`worldStore.getWorld` when a save exists, `deserializeWorldInto` for native v20 saves) construct/reset with `{ regenerate: false }` so the generator does NOT run on load.
