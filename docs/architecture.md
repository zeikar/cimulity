...
6. On growth ticks (`tickCount % ZONE_GROWTH_INTERVAL === 0`), first runs an **abandonment sweep** over every building (added service-v8 / save v18 — `Building.abandoned: boolean`): if land value at the anchor no longer supports the building's level, it flips `abandoned = true`; if a previously-abandoned building's anchor land value now supports its level again, it flips back to `false`. Either flip freezes the building for the rest of this tick — recorded in a `frozenThisTick` set, not the live `abandoned` field, so a building that just re-occupied is still skipped by growth/merge below. Abandoned buildings are excluded from population, jobs, demand, and the labor market; the sweep runs before demand is (re)computed so this tick's growth pass already sees demand net of the buildings that just went derelict. The render layer shows a derelict tint with dark windows.
   ...
   - Building already exists → gated first on `hasFrontageRoadAccess` + power (a building that loses either stops aging entirely), then one of three mutually exclusive branches, checked in this order:
     ...
     Below `ZONE_MAX_LEVEL` this ordering is bit-identical to before (structure-grow already preceded level-up there). The one behavioural change is at max level: a building whose structure can still extend now does that instead of densifying. This is reachable only after a second-generation (4-wide) merge — a first-generation (2-wide) merge's depth cap is unchanged at `max(2, 2) = 2`, so a 4-wide-and-up lot is the earliest shape whose raised `structureDepthCap` (`max(2, lot width)` = 4 for a 4-wide lot) *can* exceed its current structure depth. Whether it actually has room to extend still depends on the lot's own depth being greater than that structure depth — a shallow 4-wide lot (e.g. 2 cells deep, the shape a first-generation merge of two 2-deep lots currently produces) is already structure-capped at 2 and gains no room from the width-only merge, disproven by the previous cycle's playtest (its only 4-wide lot was 2 deep and never extended in 7797 ticks).

   Separately, after the per-tile loop, a width-axis **merge** pass considers every pair of adjacent, same-type, same-frontage buildings. `canMerge` treats merging as the LAST redevelopment rung, not a response to a demand spike: a pair merges only once both parcels are BUILT OUT — both at `ZONE_MAX_LEVEL` (5), both at the density tier their own lot width allows (`maxDensityForLot`), and both with structures that can no longer extend (`canExtendStructure` false on each side) — with demand merely positive (`> GROWTH_DEMAND_THRESHOLD`, not a density-tier spike), since a parcel that has exhausted every cheaper way to grow has nothing left to answer demand with. `canMerge` also keeps its existing geometry checks (equal lot depth, frontage-edge alignment, width-axis adjacency, 4×4 lot cap) and the equal-`structureRect`-dimensions gate. Because both sides are built out, `level` is automatically equal — nothing in the sim carries a level past `ZONE_MAX_LEVEL` (5) — which also closes an NW-anchor defect, since the merged building's anchor is always one of the two originals', whose level the abandonment sweep already verified this same tick. `density` is not automatically equal — a built-out mixed-width pair could still sit at different density tiers — so equal density instead falls out of combining the per-side density-cap gate with the equal-`structureRect`-dimensions gate (see the built-out and equal-shape gate comments in `canMerge`, `game/core/mergePolicy.ts`, for the full derivation); that same equal-dimensions gate is also what makes `buildingCapacity(merged) === buildingCapacity(a) + buildingCapacity(b)` an exact integer invariant. A merge is still capacity-*neutral* at the instant it fires (the conservation equality above), but a first-generation (2-wide) merge now unlocks density tier 2 via `maxDensityForLot`, which its 1-wide inputs could never reach alone — level-5 ceiling 200 vs. 140 for the unmerged pair (≈ +43%); second-generation (4-wide) merges can unlock additional structure depth/area instead (the raised `structureDepthCap` above) *only when the lot has unused depth beyond its current structure* — a shallow 4-wide lot (structure already filling its depth) gains no area from the merge itself; never density, regardless of depth.
