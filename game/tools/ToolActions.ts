 ...
 * POLICE_STATION, FIRE_STATION, HOSPITAL, SCHOOL, PARK.
 */
function isStructuredCell(world: World, tile: Tile, x: number, y: number): boolean {
  ...
  if (tile.type === TileType.PARK) return true;
...
function classifyRoadTile(world: World, x: number, y: number): PlaceClassification {
  ...
  if (tile.type === TileType.PARK) return 'reject';
  ...
  switch (tool) {
    ...
    case Tool.BULLDOZE: {
      ...
      for (const { x, y } of tiles) {
        ...
        // Structure tiles (POWER_PLANT, WATER_TOWER, POLICE_STATION, FIRE_STATION, HOSPITAL, SCHOOL, PARK): expand to full structure
        ...
        if (
          ...
          currentTile.type === TileType.PARK
        ) {
    ...
    case Tool.HOSPITAL: {
      // `tiles[0]` IS the NW anchor — `pathForTool(Tool.HOSPITAL, start, end)` returns
      // `[start]`, so the preview path has exactly one tile.
      if (tiles.length > 0 && classifyStructurePlacement(world, tiles[0].x, tiles[0].y, 'hospital') === 'reject') {
        rejected = [tiles[0]];
      } else {
        rejected = [];
      }
    ...
    case Tool.PARK: {
      // `tiles[0]` IS the NW anchor — `pathForTool(Tool.PARK, start, end)` returns
      // `[start]`, so the preview path has exactly one tile.
      if (tiles.length > 0 && classifyStructurePlacement(world, tiles[0].x, tiles[0].y, 'park') === 'reject') {
        rejected = [tiles[0]];
      } else {
        rejected = [];
      }
...
export function buildToolCommands(
  ...
  switch (tool) {
    ...
    case Tool.PARK:
      return buildParkCommands(dragStart, world);
    default:
      return [];
  }
}
...
function buildBulldozeCommands(tiles: TileCoord[], world: World): ToolCommand[] {
  ...
  for (const coord of tiles) {
    ...
    // Applies to all structure tile types (POWER_PLANT, WATER_TOWER, POLICE_STATION, FIRE_STATION, HOSPITAL, SCHOOL, PARK are all registered in StructureMap).
    if (
      currentTile.type === TileType.POWER_PLANT ||
      currentTile.type === TileType.WATER_TOWER ||
      currentTile.type === TileType.POLICE_STATION ||
...
function buildParkCommands(tile: TileCoord, world: World): ToolCommand[] {
  if (classifyStructurePlacement(world, tile.x, tile.y, 'park') === 'reject') return [];
  return [{ kind: 'place-structure', x: tile.x, y: tile.y, structureType: 'park' }];
}
