...
// ---------------------------------------------------------------------------
//
// FireCoverageMap.recompute delegates to propagateServiceCoverage (with a
// `fire_station` source selector). The full BFS topology matrix lives in
// serviceCoveragePropagation.test.ts — the single source of BFS truth. These
// tests cover only the class surface: source-type selection (fire only, not
// police), getCoverageNormalized, getCoverage OOB, clear(), and
// isFireAnchorCovered threshold boundary.
...
describe('FireCoverageMap', () => {
  describe('source-type selection: fire_station seeds coverage', () => {
    // Fire station at (1,1) — 2×2 footprint (1,1),(2,1),(1,2),(2,2).
    // Road at (1,0) — orthogonally adjacent to footprint cell (1,1).
    // Proves fire_station seeds the road at distance 0 → raw intensity 255.
    let svc: FireCoverageMap;
    beforeEach(() => {
      const map = makeMap(10, 10, [{ x: 1, y: 0, type: TileType.ROAD }]);
      const structures = new StructureMap(10, 10);
      placeFireStation(structures, 1, 1);
      svc = new FireCoverageMap(10, 10);
      svc.recompute(map, structures);
    });

    it('road cell adjacent to fire station at distance 0 reads 255', () => {
  ...
  describe('cross-source isolation: fire ignores police_station', () => {
    it('a police_station adjacent to road does NOT produce fire coverage', () => {
      // Road at (1,0). Police station at (1,1) — adjacent to the road.
      // FireCoverageMap uses fire_station as source predicate, so the
      // police station should NOT seed fire coverage.
      const map = makeMap(10, 10, [{ x: 1, y: 0, type: TileType.ROAD }]);
      const structures = new StructureMap(10, 10);
  ...
  describe('getCoverageNormalized', () => {
    ...
    it('returns 0 for an uncovered cell', () => {
      const svc = new FireCoverageMap(5, 5);
  ...
  describe('isFireAnchorCovered — threshold boundary', () => {
