/**
 * Graded service-coverage map for school coverage — stores intensity (0..255),
 ...
 *   This class is the FOURTH service-coverage sibling, adding the education
 *   category on top of the police+fire+hospital emergency trio. Police shipped
 *   first (`ServiceCoverageMap`); fire second (`FireCoverageMap`); hospital
 *   third (`HospitalCoverageMap`); school is this file. Hard-codes `'school'`
 *   as the source type and reuses `propagateServiceCoverage` plus the shared
 *   threshold constant `SERVICE_COVERAGE_THRESHOLD_RAW` — exactly how
 *   WaterMap clones PowerMap for `'water_tower'`.
 *
 ...
 *   at the building's anchor point. See `isSchoolAnchorCovered` below.
 */
...
export class SchoolCoverageMap {
 ...
 */
export function isSchoolAnchorCovered(
  anchor: { x: number; y: number },
  schoolSvc: SchoolCoverageMap,
): boolean {
  return schoolSvc.getCoverage(anchor.x, anchor.y) >= SERVICE_COVERAGE_THRESHOLD_RAW;
}
