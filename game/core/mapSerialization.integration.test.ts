...
describe('demand freshness on hydrate', () => {
  it('getDemand() reflects hydrated buildings immediately after deserializeWorldInto', () => {
    ...
    src.getMap().getBuildings().addExistingBuilding({ id: 0, type: 'industrial', footprint: [{ x: 1, y: 1 }], anchor: { x: 1, y: 1 }, level: 3, density: 0, age: 0, abandoned: false, frontage: 'S', structureRect: { x: 1, y: 1, w: 1, h: 1 } });
    src.getMap().getBuildings().addExistingBuilding({ id: 1, type: 'industrial', footprint: [{ x: 2, y: 1 }], anchor: { x: 2, y: 1 }, level: 3, density: 0, age: 0, abandoned: false, frontage: 'S', structureRect: { x: 2, y: 1, w: 1, h: 1 } });
