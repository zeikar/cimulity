...
// The storage key mirrors the constant in worldStore.ts (v20 cut).
...
function makeFullApiStub() {
  ...
  return {
    ...
    getElapsedDays: () => 0,
    setElapsedDays: () => {},
    getServiceFundingPerMille: () => 1000,
    setServiceFundingPerMille: () => false,
    monthlyTaxIncome: () => 0,
    monthlyUpkeep: () => 0,
    getMap: () => ({
      getBuildings: () => ({
    ...
    getWaterMap: () => null,
    markWaterDirty: () => {},
    recomputeWaterIfDirty: () => {},
    recomputeWater: () => {},
    getServiceCoverageMap: () => null,
    markServiceDirty: () => {},
...
describe('getWorld — sentinel: Test A — API probe fails → fresh World', () => {
  it('discards a stub missing required methods even when sentinel matches', () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (globalThis as any).__cimulityWorldGuard = 'service-v20';
...
describe('getWorld — sentinel: stub missing getServiceCoverageMap → fresh World', () => {
  it('discards a full-API stub that lacks getServiceCoverageMap even when the sentinel matches', () => {
    // Full API stub minus getServiceCoverageMap — simulates a pre-service singleton.
    const stale = makeFullApiStub();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    delete (stale as any).getServiceCoverageMap;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    expect(result).toBeInstanceOf(World);
    expect(typeof result.getServiceCoverageMap).toBe('function');
    expect(typeof result.markServiceDirty).toBe('function');
    expect(typeof result.recomputeServiceIfDirty).toBe('function');
    expect(typeof result.recomputeService).toBe('function');
  });
});
...
describe('getWorld — sentinel: Test D — no pre-seed → fresh World + guard set', () => {
  it('builds a fresh World and writes service-v20 to globalThis.__cimulityWorldGuard', () => {
    // Singleton and guard are already cleared by beforeEach (resetSingleton).
    const result = getWorld();

    expect(result).toBeInstanceOf(World);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    expect((globalThis as any).__cimulityWorldGuard).toBe('service-v20');
  });
});
...
describe('WORLD_SINGLETON_GUARD invalidates stale HMR singletons', () => {
  it('rebuilds when guard differs', () => {
    ...
    expect(globals.__cimulityWorldGuard).toBe('service-v20');
  });
});
...
describe('getWorld — sentinel: stub missing getHospitalCoverageMap → fresh World', () => {
  it('discards a full-API stub that lacks getHospitalCoverageMap even when the sentinel matches', () => {
    // Full API stub minus getHospitalCoverageMap — simulates a pre-hospital singleton.
    const stale = makeFullApiStub();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    delete (stale as any).getHospitalCoverageMap;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (globalThis as any).__cimulityWorld = stale;
    // Current sentinel so only the API probe causes the discard.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    expect(result).not.toBe(stale);
    expect(result).toBeInstanceOf(World);
    expect(typeof result.getHospitalCoverageMap).toBe('function');
...
describe('getWorld — sentinel: stub missing getHappiness → fresh World', () => {
  it('discards a full-API stub that lacks getHappiness even when the sentinel matches', () => {
    // Full API stub minus getHappiness — simulates a pre-happiness-KPI singleton.
    const stale = makeFullApiStub();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    delete (stale as any).getHappiness;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (globalThis as any).__cimulityWorld = stale;
    // Current sentinel so only the API probe causes the discard.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
describe('getWorld — sentinel: stub missing getServiceFundingPerMille → fresh World', () => {
  it('discards a full-API stub that lacks getServiceFundingPerMille even when the sentinel matches', () => {
    // Full API stub minus getServiceFundingPerMille — simulates a pre-budget-stakes singleton.
    const stale = makeFullApiStub();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    delete (stale as any).getServiceFundingPerMille;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (globalThis as any).__cimulityWorld = stale;
    // Current sentinel so only the API probe causes the discard.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    expect(result).not.toBe(stale);
    expect(result).toBeInstanceOf(World);
    expect(typeof result.getServiceFundingPerMille).toBe('function');
    expect(typeof result.setServiceFundingPerMille).toBe('function');
    expect(typeof result.monthlyTaxIncome).toBe('function');
    expect(typeof result.monthlyUpkeep).toBe('function');
  });
});
