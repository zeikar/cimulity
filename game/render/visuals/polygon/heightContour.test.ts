...
const HIGH = ROCK_MIN_HEIGHT; // rock threshold (4.5) — fixtures straddle it with 5/3
...
describe('thresholds', () => {
  ...
  it('ROCK_MIN_HEIGHT catches the higher ground', () => {
    expect(ROCK_MIN_HEIGHT).toBe(4.5);
  });
});
...
describe('contourPolygon — above (highland rock)', () => {
  ...
  it('returns the whole triangle when every corner is above', () => {
    expect(contourPolygon(5, 5, 5, HIGH, 'above')).toEqual([
      { kind: 'corner', i: 0 },
      { kind: 'corner', i: 1 },
      { kind: 'corner', i: 2 },
    ]);
  ...
  it('clips to a wedge at corner 0 when only corner 0 is above, interpolating by height', () => {
    // cross t = (4.5 - 5) / (3 - 5) = 0.25 along each edge leaving the peak corner.
    expect(contourPolygon(5, 3, 3, HIGH, 'above')).toEqual([
      { kind: 'corner', i: 0 },
  ...
  it('clips to a wedge at corner 1 when only corner 1 is above', () => {
    expect(contourPolygon(3, 5, 3, HIGH, 'above')).toEqual([
      { kind: 'corner', i: 1 },
  ...
  it('clips to a wedge at corner 2 when only corner 2 is above', () => {
    expect(contourPolygon(3, 3, 5, HIGH, 'above')).toEqual([
      { kind: 'corner', i: 2 },
  ...
  it('clips to a quad when corner 0 is the only one below', () => {
    expect(contourPolygon(3, 5, 5, HIGH, 'above')).toEqual([
      { kind: 'corner', i: 1 },
      { kind: 'corner', i: 2 },
      { kind: 'edge', a: 2, b: 0, t: 0.25 },
      { kind: 'edge', a: 1, b: 0, t: 0.25 },
  ...
  it('clips to a quad when corner 1 is the only one below', () => {
    expect(contourPolygon(5, 3, 5, HIGH, 'above')).toEqual([
      { kind: 'corner', i: 2 },
      { kind: 'corner', i: 0 },
      { kind: 'edge', a: 0, b: 1, t: 0.25 },
      { kind: 'edge', a: 2, b: 1, t: 0.25 },
  ...
  it('clips to a quad when corner 2 is the only one below', () => {
    expect(contourPolygon(5, 5, 3, HIGH, 'above')).toEqual([
      { kind: 'corner', i: 0 },
      { kind: 'corner', i: 1 },
      { kind: 'edge', a: 1, b: 2, t: 0.25 },
      { kind: 'edge', a: 0, b: 2, t: 0.25 },
